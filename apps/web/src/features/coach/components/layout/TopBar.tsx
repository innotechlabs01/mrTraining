    ...
    <header className="fixed top-0 left-0 right-0 h-14 bg-surface-1/85 backdrop-blur-xl border-b border-surface-3 z-20 flex items-center justify-between px-4 lg:px-6">
      <div className="flex items-center gap-3 min-w-0">
        <button
          type="button"
          onClick={onMenuClick}
          className="p-2 rounded-xl text-text-secondary hover:text-text-primary hover:bg-surface-3 transition-all active:scale-95 lg:hidden flex-shrink-0"
          aria-label="Open navigation menu"
        >
          <Menu size={20} />
        </button>
        <div className="hidden lg:block w-16 flex-shrink-0" />
        <h1 className="text-sm font-semibold text-text-primary tracking-tight flex-shrink-0">Today</h1>
        <span className="hidden md:inline-flex items-center gap-1.5 text-xs text-text-muted whitespace-nowrap truncate shrink-0">
      ...
      <div className="flex items-center gap-2 flex-shrink-0">
        {currentBlock && (
          <div className="hidden md:flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface-2 border border-surface-3 shrink-0">
