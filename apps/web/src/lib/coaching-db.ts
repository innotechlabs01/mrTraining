...
export async function saveProduct(coachId: string, data: Record<string, unknown>) {
  ...
  if (existing.rows.length > 0) {
    await db.execute(
      'UPDATE products SET name=?, brand=?, image_url=?, price=?, received=?, gross=?, stock=?, low_stock_threshold=?, updated_at=datetime(\'now\') WHERE id=? AND coach_id=?',
      [data.name, data.brand || '', data.imageUrl || '', data.price, data.received, data.gross, data.stock || 0, data.lowStockThreshold || 5, id, coachId],
    )
  } else {
    await db.execute(
      'INSERT INTO products (id, name, brand, image_url, price, received, gross, stock, low_stock_threshold, coach_id) VALUES (?,?,?,?,?,?,?,?,?,?)',
      [id, data.name, data.brand || '', data.imageUrl || '', data.price, data.received, data.gross, data.stock || 0, data.lowStockThreshold || 5, coachId],
    )
  }
  return id
}
